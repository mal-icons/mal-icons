...
A Vite + Preact app that renders the shared mal-icons gallery with live controls
for size, color, stroke weight, and animation.

It mirrors the React example, but uses Preact's lighter runtime and the
`@mal-icons/preact` adapter — same context-based theming API
...
- `import { FiActivity } from "@mal-icons/preact/fi"` — tree-shakeable, ahead-of-time
