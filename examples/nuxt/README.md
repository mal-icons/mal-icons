...
- **Source resolution** — `nuxt.config.ts` resolves the workspace `@mal-icon/*`
  packages to their TypeScript source (client and server) via the `bun` export
  condition, so the example runs without pre-building the libraries.
