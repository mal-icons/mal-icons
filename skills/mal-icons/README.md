...
Flat Color Icons `fc`, Game Icons `gi`, Grommet `gr`, Heroicons `hi`, Simple Line Icons `sl`, css.gg `cg`, Codicons `vsc`, Themify `tf`, Radix Icons `rx`, Bootstrap Icons `bs`, Material Icons Filled `mdf` / Rounded `mdr` / Sharp `mds` / Two Tone `mdt`, Material Symbols Outlined `mso` / Rounded `msr` / Sharp `mss` —
36,350 icons) with tags and descriptions, a fast search script, per-framework
