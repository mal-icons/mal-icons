...
Flat Color Icons `fc`, Grommet `gr`, Heroicons `hi`, Simple Line Icons `sl`, css.gg `cg`, Codicons `vsc`, Themify `tf`, Radix Icons `rx`, Material Icons `md` (+ Filled `mdf`, Rounded `mdr`, Sharp `mds`, Two Tone `mdt`), Material Symbols Outlined `mso` / Rounded `msr` / Sharp `mss` —
32,214 icons) with tags and descriptions, a fast search script, per-framework
