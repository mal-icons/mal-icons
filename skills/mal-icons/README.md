...
Line Awesome `la`, Lucide `lu`, Font Awesome Free `fa`, Font Awesome Free Brands `fab`, Font Awesome Free Regular `far`,
Typicons `ti`, Octicons `oc`, Devicons `dev`, Weather Icons `wi`, Ant Design `ad`,
Flat Color Icons `fc`, Game Icons `gi`, Grommet `gr`, Heroicons `hi`, Simple Line Icons `sl`, css.gg `cg`, Codicons `vsc`, Themify `tf`, Radix Icons `rx`, Remix Icon `ri`, Simple Icons `si`, Bootstrap Icons `bs`, Boxicons Regular `bx` / Solid `bxs` / Logos `bxl`, Tabler Icons `tb`, Material Icons Filled `mdf` / Rounded `mdr` / Sharp `mds` / Two Tone `mdt`, Material Symbols Outlined `mso` / Rounded `msr` / Sharp `mss`, Phosphor `ph` —
52,369 icons) with tags and descriptions, a fast search script, per-framework
...
Bundled icon sets keep their upstream licenses — Feather (MIT), Ionicons (MIT), Line Awesome (MIT), Lucide (ISC), Octicons (MIT), Devicons (MIT), Ant Design (MIT), Font Awesome Free (CC-BY-4.0), Flat Color Icons (CC-BY-4.0), Game Icons (CC-BY-3.0),
Circum (MPL-2.0), Typicons (CC-BY-SA-4.0), Weather Icons (OFL-1.1), Grommet (Apache-2.0), Remix Icon (Apache-2.0), Simple Icons (CC0-1.0), Material Icons and Material Symbols (Apache-2.0).
